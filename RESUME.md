...
- **AEM**: keys i18n `cabinUpgradeForm.*` nuevas (labels del formulario/modales) + environment keys opcionales
  ya soportadas con fallback (`AV_UPGRADES_MMB_URL`, `AV_UPGRADES_CHANNEL`).
...
- environment.json keys: AV_UPGRADES_MMB_URL, AV_UPGRADES_CHANNEL (fallback 'MMB'). NO van creds Cognito.
