...
function mockDeps({
  show, language = 'es', country = 'co', baseUrl = 'https://am', apiKey = 'dc-guid',
}) {
  ...
  vi.doMock(amPath, () => ({
    getAirMarketingConfig: vi.fn().mockResolvedValue({
      baseUrl, searchbarApiKey: 'sb', destinationCardsApiKey: apiKey,
    }),
...
describe('destination-cards-embed decorate', () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it('hides the block and renders no widget when POS does not match', async () => {
    ...
    const decorate = (await import('../../blocks/destination-cards-embed/destination-cards-embed.js')).default;
    const block = document.createElement('div');
    document.body.appendChild(block);
    await decorate(block);
    expect(hideBlockWithSection).toHaveBeenCalledWith(block);
    expect(block.querySelector('avianca-destination-cards')).toBeNull();
  });

  it('renders avianca-destination-cards with api-key, lang and pos when POS matches', async () => {
    mockDeps({
      show: true, language: 'pt', country: 'br', apiKey: 'dc-guid',
    });
    const decorate = (await import('../../blocks/destination-cards-embed/destination-cards-embed.js')).default;
    const block = document.createElement('div');
    document.body.appendChild(block);
    await decorate(block);
    const el = block.querySelector('avianca-destination-cards');
    expect(el).not.toBeNull();
    expect(el.getAttribute('api-key')).toBe('dc-guid');
    expect(el.getAttribute('lang')).toBe('pt');
    expect(el.getAttribute('pos')).toBe('br');
  });

  it('loads the destination-cards module from the configured base URL', async () => {
    ...
    document.body.appendChild(block);
    await decorate(block);
    expect(loadAirMarketingEmbed).toHaveBeenCalledWith('https://am/embedded/destination-cards.js');
  });
  ...
  it('maps internal POS country eu to es via getIataCountryCode', async () => {
    ...
    document.body.appendChild(block);
    await decorate(block);
    expect(block.querySelector('avianca-destination-cards').getAttribute('pos')).toBe('es');
  });
